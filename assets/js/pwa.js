...
/** True where the browser gives us no install API and the viewer has to be
 *  shown the menu path instead. */
const MANUAL_ONLY = IOS || MAC_SAFARI;
const STANDALONE =
...
export function initPWA({ onUpdate, onMessage } = {}) {
  ...
  if (MANUAL_ONLY) {
    ...
    button.addEventListener("click", () => {
      onMessage?.(...(IOS
        ? ["Add to Home Screen", "Tap Share in Safari, then choose “Add to Home Screen”."]
        : ["Add to Dock", "In Safari’s menu bar choose File → Add to Dock."]));
    });
    dialog.offerLater();
    return;
  }
